...
import {
  ...
  Clock, AlertCircle, TrendingUp, X, Loader2
} from "lucide-react";
...
function GoalCard({ goal, onAdd, onDelete }) {
  ...
  const progressColor = goal.isTercapai || goal.status === "tercapai"
    ? "bg-emerald-500"
    : goal.isOverdue
      ? "bg-rose-500"
      : goal.daysLeft <= 30
    ...
    <div className="bg-white rounded-2xl border border-teal-100/60 shadow-sm p-5 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold leading-tight text-zinc-900">{goal.nama}</h3>
            <p className="text-xs text-zinc-400 capitalize mt-0.5">{goal.kategori}</p>
          </div>
        </div>
      ...
      <div className="mb-4">
        <div className="flex items-end justify-between mb-1.5">
          <div>
            ...
            <p className="text-base font-bold text-zinc-900">{formatIDR(goal.currentAmount)}</p>
          </div>
          <div className="text-right">
            <p className="text-xs text-zinc-400">Target</p>
            <p className="text-sm font-semibold text-zinc-600">{formatIDR(goal.targetAmount)}</p>
          </div>
        </div>
        ...
        <div className="flex justify-between mt-1">
          <p className="text-xs font-semibold text-zinc-500">{goal.progress}%</p>
          <p className="text-xs text-zinc-400">sisa {formatIDR(goal.sisaAmount)}</p>
        </div>
      </div>
      ...
      {!goal.isTercapai && goal.status !== "tercapai" && (
        <div className="grid grid-cols-2 gap-2 mb-4">
          <div className="px-3 py-2 bg-zinc-50 rounded-xl">
            <p className="text-[10px] text-zinc-400 uppercase tracking-wide">Per Hari</p>
            <p className="text-sm font-bold text-zinc-800">{formatIDR(goal.perHari)}</p>
          </div>
          <div className="px-3 py-2 bg-zinc-50 rounded-xl">
            <p className="text-[10px] text-zinc-400 uppercase tracking-wide">Sisa Hari</p>
            <p className="text-sm font-bold text-zinc-800">{goal.daysLeft} hari</p>
          </div>
        </div>
      )}
      ...
      {(goal.isTercapai || goal.status === "tercapai") && (
        <div className="px-3 py-2 mb-4 text-center border bg-emerald-50 border-emerald-100 rounded-xl">
          <p className="text-sm font-semibold text-emerald-700">🎉 Goal tercapai! Selamat!</p>
        </div>
      )}

      {/* Add Button */}
      {goal.status !== "tercapai" && !goal.isTercapai && (
        showAdd ? (
          <div className="flex gap-2">
            ...
            >
              {loading ? <Loader2 size={14} className="animate-spin" /> : "Tambah"}
            </button>
            <button
              onClick={() => { setShowAdd(false); setAmount(""); }}
...
function CreateGoalModal({ onClose, onCreate }) {
  ...
  const handleChange = (e) => {
    ...
    if (name === "kategori") {
      ...
      setForm(prev => ({ ...prev, kategori: value, icon: kat?.icon || "🎯" }));
    }
  };
  ...
  const inputClass = "w-full px-3 py-2.5 border border-teal-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-400 bg-white";
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-teal-500">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-teal-100">Tentukan target tabunganmu</p>
            </div>
          </div>
          <button onClick={onClose} className="flex items-center justify-center rounded-full w-7 h-7 bg-white/20 hover:bg-white/30">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input name="targetAmount" type="number" value={form.targetAmount}
                onChange={handleChange} placeholder="5000000"
                className={inputClass} required min="1000" />
            </div>
            <div>
              <label className={labelClass}>Sudah punya (Rp)</label>
              <input name="currentAmount" type="number" value={form.currentAmount}
            ...
            <div className="grid grid-cols-4 gap-2">
              {KATEGORI_OPTIONS.map(kat => (
                <button key={kat.value} type="button"
                  onClick={() => setForm(prev => ({ ...prev, kategori: kat.value, icon: kat.icon }))}
...
export default function SavingGoals() {
  ...
  return (
    <DashboardLayout>
      <div className="w-full p-4 mx-auto md:p-6 max-w-7xl">
        ...
        {goals.length > 0 && (
          <div className="grid grid-cols-2 gap-4 mb-6 md:grid-cols-4">
            {[
              { label: "Total Goal",     value: goals.length,             icon: Target,      color: "text-teal-600",    bg: "bg-teal-50" },
              { label: "Goal Aktif",     value: goalAktif,                icon: TrendingUp,  color: "text-blue-600",    bg: "bg-blue-50" },
              { label: "Tercapai",       value: goalTercapai,             icon: CheckCircle2,color: "text-emerald-600", bg: "bg-emerald-50" },
              { label: "Total Terkumpul",value: formatIDR(totalTerkumpul),icon: PiggyBank,   color: "text-purple-600",  bg: "bg-purple-50" },
            ].map((stat, i) => (
              <div key={i} className="px-4 py-3 bg-white border shadow-sm rounded-xl border-teal-100/60">
        ...
        {isLoading ? (
          ...
          <div className="p-16 text-center bg-white border shadow-sm rounded-2xl border-zinc-200">
            ...
            <h3 className="mb-2 text-lg font-bold text-zinc-900">Belum ada goal tabungan</h3>
            <p className="max-w-xs mx-auto mb-6 text-sm text-zinc-500">
              Mulai tentukan target tabunganmu dan pantau progressnya setiap hari!
            </p>
            <button onClick={() => setShowModal(true)}
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {goals.map(goal => (
              <GoalCard key={goal.id} goal={goal} onAdd={handleAdd} onDelete={handleDelete} />
            ))}
          </div>
        )}
      </div>
      ...
      {showModal && (
        <CreateGoalModal onClose={() => setShowModal(false)} onCreate={handleCreate} />
      )}
    </DashboardLayout>
  );
}