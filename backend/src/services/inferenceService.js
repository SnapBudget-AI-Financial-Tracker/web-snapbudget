...
const AI_SERVICE_URL = process.env.AI_SERVICE_URL || 'https://aprizal543i--snapbudget-ai-fastapi-app.modal.run';
...
export const scanStruk = async (fileBuffer, filename, context = {}) => {
  try {
    ...
    formData.append('day_of_month', context.day_of_month || new Date().getDate());
    formData.append('saldo_sisa', context.saldo_sisa || 0);

    const response = await axios.post(`${AI_SERVICE_URL}/scan-struk`, formData, {
      headers: {
        ...formData.getHeaders(),
      },
    ...
    throw new Error(error.response?.data?.detail || 'Gagal memproses struk via AI service');
  }
};
...
export const getPrediksi = async (actualHariIni, context = {}) => {
  try {
    const response = await axios.post(`${AI_SERVICE_URL}/prediksi`, {
    ...
    return {
      ...
      rekomendasi: {
        ...
        label_upper: 'AMAN',
        confidence: 0,
        pesan: 'AI service sedang tidak tersedia. Data prediksi akan muncul setelah service aktif.',
        saldo_rp: 0,
        days_remaining: 0,
        proj_overall_pct: 0,
