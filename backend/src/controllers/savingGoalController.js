...
export const getSavingGoals = async (req, res) => {
  try {
    ...
    const goalsWithProgress = goals.map((goal) => {
      ...
      const perBulan =
        daysLeft > 0 ? sisaAmount / Math.max(1, Math.ceil(daysLeft / 30)) : 0;
...
export const createSavingGoal = async (req, res) => {
  try {
    const userId = req.user.id;
    const { nama, targetAmount, deadline, kategori, icon, currentAmount } =
      req.body;
    ...
    res.json({
      ...
      message:
        newStatus === "tercapai"
          ...
          : `Berhasil menambah Rp ${parseFloat(amount).toLocaleString(
              "id-ID"
            )}`,
    });
  } catch (error) {
